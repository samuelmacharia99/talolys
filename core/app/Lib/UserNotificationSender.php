...
class UserNotificationSender {
    ...
    public function notificationToAll($request) {
        ...
        if ($request->being_sent_to == 'filtered_users') {
            if (!session()->has('FILTERED_USERS')) {
                return $this->redirectWithNotify('error', "Your session has been destroyed. Please filter users again");
            }
        } else {
        ...
        $users    = $this->getUsers($userQuery, $request->start, $request->batch, $request);
     ...
     */
    private function getUsers($userQuery, $start, $batch, $request) {
