<?php
    ...
    function generateEnvContent(array $post, string $appKey, string $appUrl): string
    {
        $dbHost = $post['db_host'];
        $dbName = $post['db_name'];
        $dbUser = $post['db_user'];
        ...
        $appUrlHost = $parsed['host'] ?? 'localhost';
        $tenantRootDomain = $appUrlHost;
...
CENTRAL_DOMAINS={$appUrlHost}
TENANT_ROOT_DOMAIN={$tenantRootDomain}
CRON_SECRET=
STRIPE_KEY=
STRIPE_SECRET=
...
ENV;
    ...
    if (isset($_POST['db_type']) && $_POST['db_type'] == 'create-new-database') {
    ...
    if (isset($_POST['db_type']) && $_POST['db_type'] == 'create-new-database' && @$response['error'] == 'ok') {
        try {
    ...
    if (@$response['error'] == 'ok') {
        try {
            ...
            $appUrl = rtrim($_POST['url'], '/');
            $envBody = generateEnvContent($_POST, $appKey, $appUrl);
            $envPath = __DIR__ . '/../core/.env';
            if (file_put_contents($envPath, $envBody) === false) {
                throw new Exception('Could not write environment file.');
            }
        } catch (Exception $e) {
            $response['error']   = 'error';
    ...
    $sectionTitle = empty($action) ? 'Welcome' : $action;
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
	<header class="py-3 border-bottom border-primary bg--dark">
		<div class="container">
			<div class="d-flex align-items-center justify-content-between header gap-3">
				<h3 class="title mb-0">Talolys</h3>
				<h3 class="title mb-0">Installer</h3>
			</div>
		</div>
	</header>
					...
					<div class="install-item">
						...
						<div class="box-item">
							<?php
                                if ($action == 'result') {
                                    ...
                                    if (@$response['error'] == 'ok') {
                                        ...
                                        echo '<p class="text-primary lead my-5">Your Talolys installation is ready to use.</p>';
                                        ...
                                        echo '<div class="warning mt-2"><h5 class="mb-4 fw-normal">Please review your configuration and try again.</h5><a href="?action=information" class="theme-button choto me-1 mb-3">Try Again</a></div>';
							...
							<?php
                                ...
                                ?>
								<div class="item">
									<h4 class="subtitle">Welcome to Talolys</h4>
									<p>Talolys is a digital banking platform. This installer will guide you through the server requirements check, database setup, and initial configuration.</p>
								</div>
								<div class="item text-end">
									<a href="?action=requirements" class="theme-button choto">Get Started <i class="fas fa-angle-double-right"></i></a>
								</div>
							<?php
                                }
                            ?>
	...
	<footer class="py-3 text-center bg--dark border-top border-primary">
		<div class="container">
			<p class="m-0 font-weight-bold">&copy;<?php echo Date('Y') ?> Talolys. All rights reserved.</p>
		</div>
	</footer>
